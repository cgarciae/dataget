...
def load_custom_datasets(DATASETS):
    datasets_path = ((_coconut_partial(os.path.join, {1: "datasets"}, 2))(("/".join)((_coconut.operator.itemgetter(_coconut.slice(None, -1)))((_coconut.operator.methodcaller("split", "/"))(__file__)))))

    load_datasets(DATASETS, "datasets", datasets_path)


def load_plugin_datasets(DATASETS):
    datasets_path = (_coconut_partial(os.path.join, {1: "datasets"}, 2))(get_path(global_=True))
